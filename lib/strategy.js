...
function OAuth2Strategy(options, verify) {
  ...
  var oauthClientIsDynamic = (typeof options.createOAuthClient === "function");

  if (!verify) { throw new TypeError('OAuth2Strategy requires a verify callback'); }
  if (!oauthClientIsDynamic && !options.authorizationURL) { throw new TypeError('OAuth2Strategy requires a authorizationURL option'); }
  if (!oauthClientIsDynamic && !options.tokenURL) { throw new TypeError('OAuth2Strategy requires a tokenURL option'); }
  if (!oauthClientIsDynamic && !options.clientID) { throw new TypeError('OAuth2Strategy requires a clientID option'); }

  passport.Strategy.call(this);
  this.name = 'lti-oauth2';
  this._verify = verify;
  ...
  if (oauthClientIsDynamic) {
    this._createOAuthClient = options.createOAuthClient;
  }
  else {
    this._createOAuthClient = function(req, cb) { cb(null, new OAuth2(options.clientID,  options.clientSecret,
...
OAuth2Strategy.prototype.authenticate = function(req, options) {
  var self = this;
  self._createOAuthClient(req, function(error, oauth2Client, callbackURL) {
    console.log('authenticate', error, oauth2Client);
    if (error) {
    ...
    if (req.query && req.query.error) {
      if (req.query.error == 'access_denied') {
        ...
        return self.error(new AuthorizationError(req.query.error_description, req.query.error, req.query.error_uri));
      }
    }
    ...
    var meta = {
      authorizationURL: oauth2Client._authorizeUrl,
      tokenURL: oauth2Client._accessTokenUrl,
    ...
    if (req.query && req.query.code) {
      function loaded(err, ok, state) {
        ...
        oauth2Client.getOAuthAccessToken(code, params,
      ...
      if (state) {
        ...
        var parsed = url.parse(oauth2Client._authorizeUrl, true);
        utils.merge(parsed.query, params);
        parsed.query['client_id'] = oauth2Client._clientId;
